...
# 使用 Drizzle 连接数据库
Drizzle ORM 通过**数据库驱动**在你的数据库上运行 SQL 查询。
...
在底层，Drizzle 将创建一个 **node-postgres** 驱动实例，如果需要，你可以通过 `db.$client` 访问它
<Section>
```ts
import { drizzle } from "drizzle-orm/node-postgres"
...
Drizzle 在设计上原生兼容每个**边缘**或**无服务器**运行时，无论你何时需要访问无服务器数据库 - 我们都能满足你的需求
...
是的，我们也支持运行时特定的驱动，如 [Bun SQLite](/docs/connect-bun-sqlite) 或 [Expo SQLite](/docs/connect-expo-sqlite)：
<Section>
```ts
import { drizzle } from "drizzle-orm/bun-sqlite"

const db = drizzle(); // <--- 将创建一个内存数据库
