---
title: "在 Netlify Edge Functions 中使用 Drizzle 和 Neon Postgres"
date: "2025-01-30"
---
...
本教程演示如何在 [Netlify Edge Functions](https://docs.netlify.com/edge-functions/overview/) 中使用 Drizzle ORM 和 [Neon Postgres](https://neon.tech/) 数据库。
...
登录 [Neon Console](https://console.neon.tech/app/projects) 并导航到 Projects 部分。选择一个项目或点击 `New Project` 按钮创建一个新项目。

你的 Neon 项目会自带一个名为 `neondb` 的可用 Postgres 数据库。我们将在本教程中使用它。

#### 设置连接字符串变量

在 **Project Dashboard** 部分点击 `Connect` 按钮并复制你的数据库连接字符串。它应该类似于这样：

```bash
postgres://username:password@ep-cool-darkness-123456.us-east-2.aws.neon.tech/neondb?sslmode=require
```

将 `DATABASE_URL` 环境变量添加到你的 `.env` 文件中，你将使用它来连接 Neon 数据库。
